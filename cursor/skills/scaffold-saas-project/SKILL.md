...
```bash
KIT_RULES=~/source/quantabridges/qbs-dev-kit/cursor/rules

cp "$KIT_RULES/saas-global.mdc"       .cursor/rules/
cp "$KIT_RULES/github-actions.mdc"    .cursor/rules/
cp "$KIT_RULES/docker.mdc"            .cursor/rules/
```

Conditionally:
```bash
[ "$NEED_API" = true ]      && cp "$KIT_RULES/postgres-efcore.mdc"  .cursor/rules/
[ "$NEED_FRONTEND" = true ] && cp "$KIT_RULES/react-web.mdc"        .cursor/rules/
[ "$NEED_MOBILE" = true ]   && cp "$KIT_RULES/react-native.mdc"     .cursor/rules/
                               cp "$KIT_RULES/terraform-aws.mdc"    .cursor/rules/
```

For .NET architecture (use one):
```bash
# Traditional Services (default):
cp "$KIT_RULES/dotnet-api-services.mdc" .cursor/rules/dotnet-api.mdc

# CQRS (if user specified):
cp "$KIT_RULES/dotnet-api-cqrs.mdc" .cursor/rules/dotnet-api.mdc
```
...
# Install dependencies + Tailwind CSS v4
pnpm install
...
Update `src/frontend/vite.config.ts` to add the Tailwind plugin:
```typescript
...
Replace contents of `src/frontend/src/index.css` with:
```css
@import "tailwindcss";
```
...
Tell the user what was scaffolded (list created components), then:
