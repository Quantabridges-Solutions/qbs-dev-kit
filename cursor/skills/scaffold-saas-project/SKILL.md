...
description: Scaffold a new QBS SaaS project from scratch. Creates the full directory structure, copies standard templates (gitignore, cursorignore, docker-compose, GitHub Actions, Terraform), and installs cursor rules. Use when starting a new SaaS product, new project, new platform, or when the user says "new project", "scaffold", "bootstrap", or "start a new app".
---
...
- `.cursor/rules/` with all technology rules pre-installed
...
### 3. Copy cursor rules
Copy all rules from `qbs-standards/cursor-rules/` to `.cursor/rules/`:
- `saas-global.mdc` (always)
- `dotnet-api-services.mdc` → saved as `dotnet-api.mdc` (Traditional Services — default)
  OR `dotnet-api-cqrs.mdc` → saved as `dotnet-api.mdc` (CQRS — if user chose it)
- `postgres-efcore.mdc` (if has API)
- `react-web.mdc` (if has frontend)
- `react-native.mdc` (if has mobile)
- `terraform-aws.mdc` (if has infra)
- `github-actions.mdc` (always)
- `docker.mdc` (always)

**Source:** `~/source/quantabridges/qbs-standards/cursor-rules/`
